...
function Login({ handleClick }) {
	...
	return (
		<div className='w-screen h-screen mx-auto z-50 top-0 fixed flex justify-center items-center font-Vollkorn'>
			<div
				className='bg-black absolute w-screen h-screen bg-opacity-70'
				onClick={() => {
					if (!isLoading) {
						handleModal()
					}
			...
			></div>
			<div className='relative md:w-1/3 max:h-[95%] 2xl:h-[85%] flex flex-col border-2 border-black bg-gray-100 py-8 px-6'>
				<h1 className='bold text-4xl text-center'>Welcome Back</h1>
				<h2 className='text-lg text-center my-1 text-slate-600'>
					Please enter your account details
				</h2>
				<form onSubmit={formik.handleSubmit} className='mt-4 px-7'>
					<div className='mb-6'>
						<label
							htmlFor='email'
					...
					>
						{isLoading ? (
							<div className='flex justify-center'>
								...
								<h2 className='my-auto mx-2'>logining you in....</h2>
							</div>
						) : (
							'Login'
						)}
